...
#================================================================================================================================================>
#====================================================> update_child_body
def update_child_body(child_body, to_insert, position, at_start):
    if position == 0:
        if at_start == 0:
            child_body.append(to_insert[0])
            return
        elif at_start == 1:
            child_body.insert(1, to_insert[0])
            return
    
    if type(child_body) == Loop:
        update_child_body(child_body.body, to_insert, position-1, at_start)
    else:
        update_child_body(child_body[0].body, to_insert, position-1, at_start)
    return child_body

#====================================================> updated_itrator
def updated_assignment_mul(index_list,loop_iterate_of_child,levels_info, matching_loops_count, loop_count, checker):
    ret_index_list = []
    
    for i  in range(0, len(index_list)):
        ind = index_list[i]
        def recursive_index_update_mul(ind, loop_iterate_of_child, levels_info, matching_loops_count, loop_count, checker):
            if loop_count == 0:
                return ind, matching_loops_count
            
            ind.dobject, matching_loops_count = recursive_index_update_mul(ind.dobject, loop_iterate_of_child, levels_info, matching_loops_count, loop_count-1, checker)
            if matching_loops_count > 0:
                if checker - 1 >= levels_info[0]:
        ...
        ind, _ = recursive_index_update_mul(ind, loop_iterate_of_child, levels_info[i], matching_loops_count, loop_count, checker)
        ret_index_list.append(ind)
    return ret_index_list
...
def post_order_traversal(root):
    if type(root) == TensorOp:
        ...
        if (type(root.operators[0]) == TensorOp):
            ...
            if (root.operators[0].op_type == "einsum"  and root.op_type != "einsum"):
                ...
                if equal_loops != 0:
                    ...
                    updated_index = updated_assignment_mul(index_list_pp[0],loop_iterate_of_child, levels_info22[0], equal_loops, count, equal_loops)
                    ...
                    child_body = update_child_body(root.operators[0].compute[0], to_insert,equal_loops,0)
                    
                    root.compute = [child_body]
                    root.op_type = "updated"
        ...
        if (type(root.operators[1]) == TensorOp):
            ...
            #====================================================================================================================>
            #====================================================================================================> einsum Right
            if (root.operators[1].op_type == "einsum"  and root.op_type != "einsum"):
                # print("======> in einsum")
                parent_bounds2 = get_bounds(root.compute[0], [], [], [])
                child_bounds2 = get_bounds(root.operators[1].compute[0], [], [], [])
                
                # print("===> Parent/Child Bounds")
                # print(parent_bounds2)
                # print(child_bounds2, "\n")
                ...
                equal_loops2 = 0
                for a in range(0, len(parent_bounds2[0])):
                    if (parent_bounds2[0][a] == child_bounds2[0][a]) and (parent_bounds2[1][a] == child_bounds2[1][a]) and (parent_bounds2[2][a] == child_bounds2[2][a]):
                        equal_loops2 = equal_loops2 + 1
                    else:
                        break
                
                # print("N equal loop bounds: ",equal_loops2,"\n")
                
                if equal_loops != 0:
                    #====================================================> For getting the updated parent body assignment
                    body_of_child = FindBody(root.operators[1].compute[0])
                    body_of_parent = FindBody(root.compute[0])

                    loop_iterate_of_child = get_loop_iterate(root.operators[1].compute[0], [])
                    loop_iterate_of_parent = get_loop_iterate(root.compute[0], [])
                    ...
                    if (root.op_type == "updated"):
                        #child to parent

                        temp = to_string(body_of_child[0])
                        temp_ops = re.findall(r'[\+\-\*/]', temp)

                        indices_index = get_index_of_all_indices(index_list_cc)
                        levels_info22 = get_index_loop_level(loop_iterate_of_child, indices_index)

                        count = count_loops(root.operators[1].compute[0], 0)
                        one_index = to_string(index_list_pp[0][0])

                        count_square_brackets = sum(1 for char in one_index if char in '[')

                        if count_square_brackets < count:
                            count = count_square_brackets
                        
                        updated_index = updated_assignment_mul(index_list_cc[0],loop_iterate_of_parent, levels_info22[0], equal_loops, count, equal_loops)
                        # PrintCCode(updated_index)
                        ...
                        # print(to_string(new_assignment))

                        replace_parent_body(root.operators[1].compute[0], new_assignment)
                        # ====================================================>
                        to_insert = root.operators[1].compute[0]
                        for b in range(0, equal_loops):
                            if(type(to_insert) == Loop):
                                to_insert = to_insert.body
                            else:
                        ...
                        new_parent_body = update_child_body(root.compute[0], to_insert, equal_loops, 1)
                        
                        root.compute = [new_parent_body]
                        root.operators[1].compute = []
                        ...
                        count = count_loops(root.compute[0], 0)
                        one_index = to_string(index_list_cc[0][0])
                        ...
                        child_body = update_child_body(root.operators[1].compute[0], to_insert,equal_loops)
...
def test1():
    ...
    print("==============================================================================================================================>")
    print("==============================================================================================================================>")
    print("===============================================================> res_with_ir [IR] [Test 1 [+] (Original)]")
    print(code0)
    new_res_with_ir = fuse(res_with_ir)
    
    print("\n==============================================================================================================================>")
    print("===============================================================> Updated IR Complete [Test 1 [+] (Original)]")
    code1 = codegen.cpu.gen_cpp(new_res_with_ir)
    print(code1)
    print("\n==============================================================================================================================>")
    print("===============================================================> Updated IR Loops Only [Test 1 [+] (Original)]")
    PrintCCode(new_res_with_ir.compute)

################################################################################################################################################## Test 2 A @ B + C @ D (Original)
def test2():
    A = Tensor('a', (20, 30))
    B = Tensor('b', (30, 40))
    C = Tensor('c', (20, 40))
    ...
    print("==============================================================================================================================>")
    print("==============================================================================================================================>")
    print("===============================================================> res_with_ir [IR] [Test 2 A @ B + C @ D (Original)]")
    print(code)
    new_res_with_ir = fuse(res_with_ir)

    print("\n==============================================================================================================================>")
    print("===============================================================> Updated IR Complete [Test 2 A @ B + C @ D (Original)]")
    code1 = codegen.cpu.gen_cpp(new_res_with_ir)
    print(code1)
    print("\n==============================================================================================================================>")
    print("===============================================================> Updated IR Loops Only [Test 2 A @ B + C @ D (Original)]")
    PrintCCode(new_res_with_ir.compute)
...
def test3():
    ...
    print("==============================================================================================================================>")
    print("==============================================================================================================================>")
    print("===============================================================> res_with_ir [IR] [Test 3 [+] Extended]")
    print(code0)
    new_res_with_ir = fuse(res_with_ir)
    
    print("\n==============================================================================================================================>")
    print("===============================================================> Updated IR Complete [Test 3 [+] Extended]")
    code1 = codegen.cpu.gen_cpp(new_res_with_ir)
    print(code1)
    print("\n==============================================================================================================================>")
    print("===============================================================> Updated IR Loops Only [Test 3 [+] Extended]")
    PrintCCode(new_res_with_ir.compute)
...
def test4():
    ...
    print("==============================================================================================================================>")
    print("==============================================================================================================================>")
    print("===============================================================> res_with_ir [IR] [Test 4 [-, /, +] Combinations]")
    print(code0)
    new_res_with_ir = fuse(res_with_ir)
    
    print("\n==============================================================================================================================>")
    print("===============================================================> Updated IR Complete [Test 4 [-, /, +] Combinations]")
    code1 = codegen.cpu.gen_cpp(new_res_with_ir)
    print(code1)
    print("\n==============================================================================================================================>")
    print("===============================================================> Updated IR Loops Only [Test 4 [-, /, +] Combinations]")
    PrintCCode(new_res_with_ir.compute)

################################################################################################################################################## Test 5 [*, +] with 2 loops equal
def test5():
    A = Tensor('a', (30, 30))
    B = Tensor('b', (30, 30))
    C = Tensor('c', (30, 30))
    ...
    print("==============================================================================================================================>")
    print("==============================================================================================================================>")
    print("===============================================================> res_with_ir [IR] [Test 5 [*, +] with 2 loops equal]")
    print(code0)
    new_res_with_ir = fuse(res_with_ir)
    
    print("\n==============================================================================================================================>")
    print("===============================================================> Updated IR Complete [Test 5 [*, +] with 2 loops equal]")
    code1 = codegen.cpu.gen_cpp(new_res_with_ir)
    print(code1)
    print("\n==============================================================================================================================>")
    print("===============================================================> Updated IR Loops Only [Test 5 [*, +] with 2 loops equal]")
    PrintCCode(new_res_with_ir.compute)

################################################################################################################################################## Test 6 A @ B + C [From Notion]
def test6():
    A = Tensor('a', (10, 20))
    B = Tensor('b', (20, 30))
    C = Tensor('c', (10, 30))
    ...
    print("==============================================================================================================================>")
    print("==============================================================================================================================>")
    print("===============================================================> res_with_ir [IR] [Test 6 A @ B + C [From Notion]]")
    print(code)
    new_res_with_ir = fuse(res_with_ir)

    print("\n==============================================================================================================================>")
    print("===============================================================> Updated IR Complete [Test 6 A @ B + C [From Notion]]")
    code1 = codegen.cpu.gen_cpp(new_res_with_ir)
    print(code1)
    print("\n==============================================================================================================================>")
    print("===============================================================> Updated IR Loops Only [Test 6 A @ B + C [From Notion]]")
    PrintCCode(new_res_with_ir.compute)

################################################################################################################################################## Test 7 A @ B + C [2 equal Loops]
def test7():
    A = Tensor('a', (20, 20))
    B = Tensor('b', (20, 20))
    C = Tensor('c', (20, 20))
    ...
    print("==============================================================================================================================>")
    print("==============================================================================================================================>")
    print("===============================================================> res_with_ir [IR] [Test 7 A @ B + C [2 equal Loops]]")
    print(code)
    new_res_with_ir = fuse(res_with_ir)

    print("\n==============================================================================================================================>")
    print("===============================================================> Updated IR Complete [Test 7 A @ B + C [2 equal Loops]]")
    code1 = codegen.cpu.gen_cpp(new_res_with_ir)
    print(code1)
    print("\n==============================================================================================================================>")
    print("===============================================================> Updated IR Loops Only [Test 7 A @ B + C [2 equal Loops]]")
    PrintCCode(new_res_with_ir.compute)
